...
if __name__ == "__main__":
    ...
    model_path = "model_data/segformer_b1_backbone_weights.pth"
    #------------------------------#
    #   输入图片的大小
    #------------------------------#
    input_shape     = [400, 400]
    ...
    river_path = r"G:\yemaolin\segformer-pytorch-master\datasets_all"
    # river_path = r"G:\yemaolin\segformer-pytorch-master\datasets_train"
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(river_path, "ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(river_path, "ImageSets/Segmentation/val.txt"),"r") as f:
    ...
    if True:
        ...
        train_dataset   = SegmentationDataset(train_lines, input_shape, num_classes, True, river_path)
        val_dataset     = SegmentationDataset(val_lines, input_shape, num_classes, False, river_path)
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines,river_path, log_dir, Cuda, \
